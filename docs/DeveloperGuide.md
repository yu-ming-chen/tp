...
depending on the state the user is currently in.

There are two types of parsers:
1. Page Parsers
2. Command Parsers

These parsers are part of the `Logic` component as seen in Figure 4.2.1 below:
...
It identifies the first word to be the command word and uses it to determine the type of `Command` that will be generated.
...
User inputs are parsed by the `MainPageParser` if it is entered when NUSave is on a main page
User inputs are parsed by the `BudgetPageParser` if it is entered while NUSave is on a budget page.
...
Arguments in a user command are parsed by a command parser to generate the respective `Command`.
The type of `Command` generated by a command parser follows the generic type of `Parser<T>` that it implements.
For example, `AddExpenditureCommandParser` implements the `Parser<AddExpenditureCommand>` interface. Hence, it will only generate `AddExpenditureCommand`.
A `ParseException` is thrown if the necessary arguments to generate the respective `Command` are invalid or missing.

#### 4.2.3. Interaction Between Parsers

The interaction between the parsers are seen in Figure 4.2.3.1 below:
